...
var e = class {
	...
	#r = this.#a.bind(this);
	constructor(e) {
		this.#t = this.#w(this.#t, e), this.#S(), this.#i(), this.#t.options.removeTabPanelTitle && this.#v();
	}
	destroy() {
	...
	#o(e) {
		...
		switch (e.key) {
			...
			case "Home":
				this.#l(t), n = !0;
				break;
			case "End": this.#u(t), n = !0;
		}
		n && (e.stopPropagation(), e.preventDefault());
	}
	#s(e) {
		let t = this.#e.tabsNavBtn, n = this.#g(t, e), r = this.#f(n, t);
		this.#d(e, r);
	}
	#c(e) {
		let t = this.#e.tabsNavBtn, n = this.#g(t, e), r = this.#p(n, t);
		this.#d(e, r);
	}
	#l(e) {
	...
	#d(e, t) {
		...
		e.setAttribute("aria-selected", "false"), e.tabIndex = -1, t.setAttribute("aria-selected", "true"), t.tabIndex = 0, t.focus(), this.#h(n, r);
	}
	#f(e, t) {
	...
	#_() {
		if (this.#t.options.useCustomNav) this.#x();
		else {
			if (this.#e.tabsNavContainer.length === 0) throw Error("[tabs plugin] tabsNavContainer element should exist.");
			this.#e.tabsNavContainer[0].innerHTML = this.#b();
		}
		this.#C("tabsNavBtn", document.querySelectorAll(`#${this.#t.contextID} [role = "tab"]`));
	}
	#v() {
	...
	#b() {
		...
		for (let e = 0; e < this.#e.tabPanelTitle.length; e++) {
			...
			r += `<button id="${i}" class="${t}" role="tab" aria-selected="${a ? "true" : "false"}" aria-controls="${n}">${this.#y(e)}</button>`;
		}
		return r += "</div>", r;
	}
	#x() {
	...
	#w(e, t) {
		let n = { ...e };
		for (let r in t) t.hasOwnProperty(r) && (n[r] = Array.isArray(t[r]) && Array.isArray(e[r]) ? e[r].concat(t[r]) : t[r] instanceof Object && e[r] instanceof Object ? this.#w(e[r], t[r]) : t[r]);
		return n;
	}
};
//#endregion
